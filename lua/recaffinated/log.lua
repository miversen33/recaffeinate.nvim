...
local CONSTANTS = {
    LOG_LOCATION = vim.fn.stdpath('log') .. '/recaffinated.log',
    ...
    LEVELS = {
        CRITICAL = {
            map_level = vim.log.levels.ERROR,
            real_level = 0,
            name = "CRITICAL",
            display = true
        },
        ...
        WARN = {
            map_level = vim.log.levels.WARN,
            real_level = 2,
            name = "WARN",
        },
        INFO = {
            map_level = vim.log.levels.INFO,
            real_level = 3,
            name = "INFO",
        },
        DEBUG = {
            map_level = vim.log.levels.DEBUG,
            real_level = 4,
            name = "DEBUG",
        },
        TRACE = {
            map_level = vim.log.levels.TRACE,
            real_level = 5,
            name = "TRACE",
        },
        TRACE2 = {
...
function M.log(message, level, source_offset, notify)
    if not M.__dun then
        M.init()
    end
    ...
    if notify or level.display then
        vim.notify(string.format("[Recaffinated.nvim]: %s", message), level.map_level)
    end
...
--- We are purposefully omitting criticaln as that already notifies

function M.errorn(...)
    M.log(join_log_parts(...), CONSTANTS.LEVELS.ERROR, CONSTANTS.SOURCE_OFFSET, true)
end

function M.warnn(...)
    M.log(join_log_parts(...), CONSTANTS.LEVELS.WARN, CONSTANTS.SOURCE_OFFSET, true)
end

function M.infon(...)
    M.log(join_log_parts(...), CONSTANTS.LEVELS.INFO, CONSTANTS.SOURCE_OFFSET, true)
end

function M.debugn(...)
    M.log(join_log_parts(...), CONSTANTS.LEVELS.DEBUG, CONSTANTS.SOURCE_OFFSET, true)
end

function M.tracen(...)
    M.log(join_log_parts(...), CONSTANTS.LEVELS.TRACE, CONSTANTS.SOURCE_OFFSET, true)
end

function M.trace2n(...)
    M.log(join_log_parts(...), CONSTANTS.LEVELS.TRACE2, CONSTANTS.SOURCE_OFFSET, true)
end

function M.trace3n(...)
    M.log(join_log_parts(...), CONSTANTS.LEVELS.TRACE3, CONSTANTS.SOURCE_OFFSET, true)
end
